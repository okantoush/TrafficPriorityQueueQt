...
CarItem::CarItem(Node* node, int dir, int lane)
{
    ...
    if (node->isEmergency)
        ...
        setBrush(QColor(30, 144, 255));  // distinct "turning" blue
    else
        setBrush(QColor(20, 60, 180));   // normal dark blue
...
bool CarItem::moveForward() {
    ...
    qreal angle = currentEdge->getPath().angleAtPercent(t);
    setRotation(-angle);

    QPointF pos = currentEdge->getPath().pointAtPercent(t); //finds where vehicle should be positioned
    setPos(pos); //positions vehicle at calculated location

    if (t >= 1.0) //checks if vehicle has reached end of edge
    {
        GraphNode* endNode= currentEdge->getEndNode(); // finds the node at the end of the current one

        vector<Edge*> nextEdges = endNode->getOutgoingEdges(); //finds edges leaving found node

        if (!nextEdges.empty())
        {
            currentEdge = nextEdges[0]; //moves vehicle to next edge from found node
            t = 0.0; //resets t to 0, to restart same process
        }
    }
